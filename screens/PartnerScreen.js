...
import {
  ...
  Animated, Modal, TextInput, Alert,
} from 'react-native';
...
function useNeonBlink(colors) {
  ...
  const neonBg = blink.interpolate({ inputRange: [0, 1], outputRange: [colors.success, '#5CF291'] });
  return { neonActive, setNeonActive, neonBg };
}

function SparkButton({ label, emoji, info, pressing, onPress, onDelete, colors, styles }) {
  ...
  useEffect(() => {
    if (isIgnited && info.ignited_at) {
      ...
      if (elapsed < NEON_DURATION_MS) {
        setNeonActive(true);
        const t = setTimeout(() => setNeonActive(false), NEON_DURATION_MS - elapsed);
  ...
  return (
    <AnimatedTouchable
      style={[
        ...
        isIgnited && { backgroundColor: neonActive ? neonBg : colors.success, borderColor: colors.success },
      ]}
      onPress={() => { if (!isIgnited) onPress(); }}
      disabled={pressing}
      activeOpacity={0.8}
    >
      <Text style={styles.emoji}>{emoji}</Text>
      <Text style={[styles.buttonText, isIgnited && styles.buttonTextIgnited]}>{label}</Text>
      {isPendingByMe && <Text style={styles.pendingLabel}>Kıvılcım çakıldı</Text>}
      ...
      {onDelete && (
        ...
        >
          <Text style={[styles.deleteBadgeText, isIgnited && { color: '#fff' }]}>×</Text>
        </TouchableOpacity>
      )}
      {timerLabel && (
...
function FoodChoiceButton({ colors, styles }) {
  ...
  const [foodStatus, setFoodStatus] = useState({ status: 'none', my_choice: null, partner_choice: null });
  ...
  useFocusEffect(useCallback(() => { loadFoodStatus(); }, [loadFoodStatus]));
  ...
  const handleChoose = async (choiceKey) => {
    ...
    const { data, error } = await supabase.rpc('choose_food', { p_choice: choiceKey });
    setChoosing(null);
  ...
  const partnerLabel = FOOD_OPTIONS.find((o) => o.key === foodStatus.partner_choice)?.label;

  return (
    <View style={styles.foodCard}>
      <TouchableOpacity style={styles.foodHeader} onPress={() => setExpanded((e) => !e)} activeOpacity={0.8}>
        ...
        <>
          <View style={styles.foodOptionsRow}>
            {FOOD_OPTIONS.map((opt) => {
              ...
              const isWaiting = isMine && foodStatus.status === 'waiting_partner';
              return (
                <TouchableOpacity
                  key={opt.key}
                  style={[
                    ...
                    isMatch && { backgroundColor: colors.success, borderColor: colors.success },
                    isMismatch && { backgroundColor: colors.danger, borderColor: colors.danger },
                ...
                >
                  ...
                  <Text style={[styles.foodLabel, (isMatch || isMismatch) && { color: '#fff' }]}>{opt.label}</Text>
                </TouchableOpacity>
              );
            })}
          </View>
          ...
          {foodStatus.status === 'match' && (
            <Text style={[styles.foodHint, { color: colors.success }]}>Eşleşti! 🎉</Text>
          )}
...
export default function PartnerScreen() {
  ...
  useEffect(() => {
    loadStatus();
    loadCustomButtons();
    pollRef.current = setInterval(() => { loadStatus(); loadCustomButtons(); }, POLL_INTERVAL_MS);
    return () => clearInterval(pollRef.current);
  ...
  const handlePress = async (buttonType) => {
    ...
    if (!error && data.success) {
      ...
      Alert.alert('Gönderilemedi', 'Bağlantı sorunu oldu, tekrar dene.');
    }
  };

  const handleCustomPress = async (buttonId) => {
    setPressing(buttonId);
    const { data, error } = await supabase.rpc('press_custom_spark', { p_button_id: buttonId });
    setPressing(null);
    if (!error && data.success) {
      if (data.status === 'ignited') {
        await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success);
  ...
  const handleAddCustomButton = async (label) => {
    ...
    const { data, error } = await supabase.rpc('add_custom_button', { p_label: label });
    if (!error && data.success) {
      await loadCustomButtons();
      return;
    }
    const messages = {
      invalid_label: 'Buton adı boş olamaz veya 60 karakterden uzun olamaz.',
      limit_reached: 'En fazla 15 özel buton ekleyebilirsiniz.',
      rate_limited: 'Çok hızlı buton ekliyorsunuz, biraz sonra tekrar deneyin.',
      no_partner: 'Önce partnerinle eşleşmelisin.',
    };
    Alert.alert('Buton eklenemedi', messages[data?.error] || error?.message || 'Bir şeyler ters gitti.');
  };
  ...
  const confirmDeleteCustomButton = (buttonId) => {
    Alert.alert('Butonu silmek istiyor musunuz?', '', [
      { text: 'Hayır', style: 'cancel' },
      {
        ...
        style: 'destructive',
        onPress: async () => {
          const { error } = await supabase.rpc('delete_custom_button', { p_button_id: buttonId });
  ...
  if (hasPartner === null) {
    return (
      <ScrollView contentContainerStyle={[styles.container, { paddingTop: insets.top + 16 }]}>
        ...
        {[0, 1, 2, 3].map((i) => (
          <Skeleton key={i} height={70} borderRadius={14} style={{ marginBottom: 14 }} />
        ))}
      </ScrollView>
    );
  }
...
function createStyles(colors) {
  return StyleSheet.create({
    ...
    centered: { flexGrow: 1, justifyContent: 'center', alignItems: 'center', padding: 40, backgroundColor: colors.background },
    title: { fontSize: 26, fontWeight: '700', marginBottom: 20, color: colors.textPrimary },
    emptyText: { textAlign: 'center', fontSize: 16, color: colors.textSecondary },
    button: {
      backgroundColor: colors.surface, borderRadius: 16, padding: 20, marginBottom: 14,
      minHeight: 76, justifyContent: 'center', borderWidth: 1, borderColor: colors.border,
    },
    buttonPending: { backgroundColor: colors.accentMuted, borderColor: colors.accentSoft },
    emoji: { fontSize: 22, marginBottom: 6 },
    buttonText: { fontSize: 16, fontWeight: '600', color: colors.textPrimary, paddingRight: 30 },
    buttonTextIgnited: { color: '#fff' },
    pendingLabel: { fontSize: 12, color: colors.accent, fontWeight: '600', marginTop: 6 },
    ignitedLabel: { fontSize: 12, color: '#fff', fontWeight: '700', marginTop: 6 },
    timerBadge: {
      position: 'absolute', right: 14, bottom: 12,
      backgroundColor: 'rgba(0,0,0,0.18)', borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3,
    },
    timerText: { fontSize: 12, fontWeight: '700', color: colors.textPrimary },
    deleteBadge: {
      position: 'absolute', top: 10, right: 10,
      width: 26, height: 26, borderRadius: 13,
      backgroundColor: 'rgba(0,0,0,0.12)', justifyContent: 'center', alignItems: 'center',
    },
    deleteBadgeText: { fontSize: 16, fontWeight: '700', color: colors.textSecondary, lineHeight: 18 },
    foodCard: {
    ...
    foodOption: {
      width: '47%', borderWidth: 1, borderColor: colors.border, borderRadius: 12,
    ...
    foodHint: { fontSize: 12, color: colors.textSecondary, paddingHorizontal: 16, paddingBottom: 16 },
    addButton: {
    ...
    addButtonText: { fontSize: 22, color: colors.textSecondary, fontWeight: '600' },
    ...
    modalInput: {
      ...
      padding: 14, fontSize: 16, color: colors.textPrimary, backgroundColor: colors.background, marginBottom: 16,
    },
    ...
    modalAddButton: { backgroundColor: colors.accent, borderRadius: 10, paddingVertical: 10, paddingHorizontal: 20 },
